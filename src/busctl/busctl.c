...
static int parse_boolean_argument(const char *optname, const char *s, bool *ret) {
        int r;
...
static int parse_json_argument(const char *s, JsonFormatFlags *ret) {
        assert(s);
        assert(ret);
...
static int list_bus_names(int argc, char **argv, void *userdata) {
        ...
        return table_print_hdr(table, arg_json_format_flags, arg_legend);
}
...
static void print_subtree(const char *prefix, const char *path, char **l) {
        ...
        const char
                *vertical = strjoina(prefix, "| "),
                *space = strjoina(prefix, "  ");
...
static int member_compare_funcp(const void *a, const void *b) {
        return member_compare_func(*((const Member **)a), *((const Member **)b));
}
...
static int status(int argc, char **argv, void *userdata) {
        ...
        if (!isempty(argv[1])) {
                ...
                if (r >= 0)
                        printf("BusID=" SD_ID128_FORMAT_STR "\n",
...
static int message_append_cmdline(sd_bus_message *m, const char *signature, char ***x) {
        ...
        for (;;) {
                ...
                case SD_BUS_TYPE_BYTE: {
                        ...
                        lv = strtoul(v, &end, 10);
                        if (!end || *end || (lv > UINT8_MAX)) {
                                if (lv > UINT8_MAX) errno = ERANGE;
                ...
                case SD_BUS_TYPE_INT16: {
                        ...
                        lv = strtol(v, &end, 10);
                        if (!end || *end || (lv > INT16_MAX) || (lv < INT16_MIN)) {
                                if ((lv > INT16_MAX) || (lv < INT16_MIN)) errno = ERANGE;
                ...
                case SD_BUS_TYPE_UINT16: {
                        ...
                        lv = strtoul(v, &end, 10);
                        if (!end || *end || (lv > UINT16_MAX)) {
                                if (lv > UINT16_MAX) errno = ERANGE;
                ...
                case SD_BUS_TYPE_INT32: {
                        ...
                        lv = strtol(v, &end, 10);
                        if (!end || *end || (lv > INT32_MAX) || (lv < INT32_MIN)) {
                                if ((lv > INT32_MAX) || (lv < INT32_MIN)) errno = ERANGE;
                ...
                case SD_BUS_TYPE_UINT32: {
                        ...
                        lv = strtoul(v, &end, 10);
                        if (!end || *end || (lv > UINT32_MAX)) {
                                if (lv > UINT32_MAX) errno = ERANGE;
                ...
                case SD_BUS_TYPE_INT64: {
                        ...
                        lv = strtol(v, &end, 10);
                        if (!end || *end || (lv > INT64_MAX) || (lv < INT64_MIN)) {
                                if ((lv > INT64_MAX) || (lv < INT64_MIN)) errno = ERANGE;
                ...
                case SD_BUS_TYPE_UINT64: {
                        ...
                        lv = strtoul(v, &end, 10);
                        if (!end || *end || (lv > UINT64_MAX)) {
                                if (lv > UINT64_MAX) errno = ERANGE;
...
static int help(void) {
        printf("%s [OPTIONS...] COMMAND ...\n\n"
               ...
               "\nSee busctl(1) for more information.\n",
...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "hH:M:C:J:qjl", options, NULL)) >= 0)
                ...
                case ARG_VERSION:
                        printf("busctl (tangle) " PROJECT_VERSION_FULL "\n");
                        return 0;
