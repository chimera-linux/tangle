...
static int table_data_compare(const void *ap, const void *bp, void *tp) {
        int r;
        const size_t *a = ap, *b = bp;
        Table *t = tp;
...
int table_print(Table *t, FILE *f) {
        ...
        if (t->sort_map) {
                ...
                qsort_r(sorted, n_rows, sizeof(size_t), table_data_compare, t);
        }
        ...
        for (unsigned pass = 0; pass < 2; pass++) {
                        ...
                        for (size_t j = 0; j < display_columns; j++) {
                                ...
                                if (r > 0) { /* Truncated because too many lines? */
                                        ...
                                        req_width = MAX(req_width,
                                                        (size_t)(utf8_console_width(last) +
                                                        utf8_console_width("...")));
                                }
                ...
                if (t->width != 0 && t->width != SIZE_MAX)
                        table_effective_width = t->width;
                         ...
                         ((pass > 0 || !any_soft) && !isatty(STDOUT_FILENO)))
                        ...
                        for (size_t j = 0; j < display_columns; j++) {
                                ...
                                if (l > width[j]) {
                                        ...
                                        if (l < width[j]) {
                                                ...
                                                aligned = align_string_mem(field, width[j], d->align_percent);
                                                if (!aligned)
                                                        return -ENOMEM;
