...
static _Thread_local sd_event *default_event = NULL;
...
static void source_disconnect(sd_event_source *s) {
        ...
        case SOURCE_CHILD:
                ...
                if (s->child.pid > 0) {
                        ...
                        (void) hashmap_remove(s->event->child_sources, (void *)(uintptr_t)(s->child.pid));
                }
                        ...
                        if (!s->child.exited) {
                                ...
                                if (s->child.pidfd >= 0) {
                                        if (pidfd_send_signal(s->child.pidfd, SIGKILL, NULL, 0) < 0) {
                                                if (errno == ESRCH) /* Already dead */
                                                        ...
                                                        log_debug_errno(errno, "Failed to kill process %u via pidfd_send_signal(), re-trying via kill(): %m",
                                                                        (unsigned)s->child.pid);
                                        } else
                                                sent = true;
                                }
                                ...
                                if (!sent)
                                        if (kill(s->child.pid, SIGKILL) < 0)
                                                if (errno != ESRCH) /* Already dead */
                                                        log_debug_errno(errno, "Failed to kill process %u via kill(), ignoring: %m",
                                                                        (unsigned)s->child.pid);
                        }
...
static void initialize_perturb(sd_event *e) {
        ...
        if (sd_id128_get_boot(&id) >= 0)
                ...
                e->perturb = 0; /* This is a super early process without /proc ?? */
}
...
_public_ int sd_event_add_child(
        ...
        if (hashmap_contains(e->child_sources, (void *)(uintptr_t)(pid)))
                return -EBUSY;
        ...
        r = hashmap_put(e->child_sources, (void *)(uintptr_t)(pid), s);
        if (r < 0)
                return r;
...
static void event_log_delays(sd_event *e) {
        ...
        for (i = 0; i < ELEMENTSOF(e->delays); i++) {
                int v = snprintf(p, l, "%u ", e->delays[i]);
                p += v;
                l -= v;
                e->delays[i] = 0;
        }
...
_public_ int sd_event_source_set_memory_pressure_type(sd_event_source *s, const char *ty) {
        ...
        if (strcmp(ty, "some") && strcmp(ty, "full"))
                return -EINVAL;
        ...
        if (!b)
                return -ENOMEM;
        if (strcmp(b, "some") && strcmp(b, "full"))
                return -EINVAL;
